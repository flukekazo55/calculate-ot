...
import { LEGACY_OT_PAYLOAD } from './legacy-seed-data';
...
const LEGACY_SEED_STORAGE_KEY = 'otLegacySeedV1';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.seedLegacyDataIfNeeded();
    this.loadLocalCache();
    if (this.records.length > 0) {
      // Local-first mode: keep localStorage as source of truth and mirror to backend.
      this.saveData();
      return;
    }
    this.loadData();
  ...
  private applyPayload(payload: unknown, persist = false): void {
    const data = this.normalizePayload(payload);
    this.records = data.records;
    this.lastUpdate = data.lastUpdate;
    if (persist) this.persistLocalCache();
  }
  ...
  private loadLocalCache(): void {
    try {
      const localRecords = this.safeLocalStorageGet('otRecords');
      const parsed = localRecords ? JSON.parse(localRecords) : [];
      this.records = Array.isArray(parsed) ? parsed : [];
    } catch {
      this.records = [];
    }
    this.lastUpdate = this.safeLocalStorageGet('otLastUpdate') || '';
  }

  private seedLegacyDataIfNeeded(): void {
    if (this.safeLocalStorageGet(LEGACY_SEED_STORAGE_KEY) === '1') {
      return;
    }

    const rawRecords = this.safeLocalStorageGet('otRecords');
    if (rawRecords) {
      try {
        const parsed = JSON.parse(rawRecords);
        if (Array.isArray(parsed) && parsed.length > 0) {
          this.safeLocalStorageSet(LEGACY_SEED_STORAGE_KEY, '1');
          return;
        }
      } catch {}
    }

    this.safeLocalStorageSet('otRecords', JSON.stringify(LEGACY_OT_PAYLOAD.records));
    this.safeLocalStorageSet('otLastUpdate', LEGACY_OT_PAYLOAD.lastUpdate);
    this.safeLocalStorageSet(LEGACY_SEED_STORAGE_KEY, '1');
  }

  private persistLocalCache(): void {
    this.safeLocalStorageSet('otRecords', JSON.stringify(this.records));
    this.safeLocalStorageSet('otLastUpdate', this.lastUpdate);
  }
